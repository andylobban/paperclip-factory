...
function buildPipelineCaseContextMarkdown(input: {
  ...
  return [
    ...
    "- Use deterministic `idempotencyKey` values for child tasks based on the semantic output and its explicit revision; reuse the same key when re-entering this case and stage, and use a new key only for a changed revision with provenance in the task description.",
